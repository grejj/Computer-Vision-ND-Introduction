...
# Implementation of Histograms of Oriented Graphs (HOG) Algorithm

# 1. Set ROI that covers entire object in image
# 2. Calculate mag and angle of gradient for each pixel in ROI
# 3. Divide detection window into connected cells of pixels to match scale of features
# 4. Create histogram for each cell (mag vs. angle)
# 5. Group adjacent cells into blocks (better performance if blocks overlap)
# 6. Use cells in block to normalize cell histograms in that block.
# 7. From HOG descriptor from all normalized histograms
# 8. Use HOG descriptor to train ML algorithm
# 9. Detect object by looking for part of image similar to HOG pattern
